...
[![Versão](https://img.shields.io/badge/versão-1.9.24-7c3aed?style=for-the-badge)](https://github.com/ALN2025/ModVozALN.exe)
...
> **Como saber se baixou a versão certa:** dentro do ZIP, abra `VERSION.txt` — deve mostrar **v1.9.24**. O `ModVozALN.exe` deve ter **~39 MB** (versão antiga ~32 MB não instala voice).

### Conteúdo do ZIP (v1.9.24)
...
Ao abrir o instalador, a splash mostra **ScriptClean** · **Modo de voz ALN**. O título da janela deve exibir **`v1.9.24`** com **4 campos**: Game, Libs, Voice, System.
...
5. Confirme no log do instalador: `ModVozALN v1.9.24` e `[Pack] OK`.
...
- [ ] `VERSION.txt` no ZIP e log do instalador mostram **v1.9.24**
...
O bridge descobre sozinho **World**, **Player** e **posição** lendo o JAR do GameServer — **sem source** da revisão. O instalador **v1.9.24+** faz o mesmo: lê o `.bat` da pack, escaneia os JARs e gera `INICIAR-GS-COM-VOZ.bat` + `fork.*` para **qualquer** revisão L2J (aCis, L2jMega, Mobius, Frozen, L2Off, pack customizada com outro pacote Java).
...
Problemas comuns: IP errado, firewall, ZIP antigo na pasta Downloads (apague e baixe de novo), `VERSION.txt` com versão antiga, cliente aberto pelo `L2.exe` em vez do `L2VoiceInject.exe`, Redis parado, **`INICIAR-GS-COM-VOZ.bat` antigo** (erro `com.l2jmega.gameserver.GameServer` em pack aCis — reinstale com v1.9.24+ para regenerar o `.bat`).
